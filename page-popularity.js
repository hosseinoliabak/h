(function () {
  ...
  // Download all three scripts in parallel but let the browser execute them
  // in the order they were appended (script.async = false preserves this).
  // Loading them one-at-a-time serially tripled the network latency before a
  // write could fire, so a quick click to the next page could cancel the
  // pending request before Firebase even loaded.
  function loadScriptsThen(callback) {
    var remaining = FIREBASE_SCRIPTS.length;
    var failed = false;
  ...
  function recordView(db, key) {
    ...
    db.ref("pageviews/" + key).set(firebase.database.ServerValue.increment(1))
      .then(function () {
        ...
        console.log("page-popularity: recorded view for", key);
      })
      .catch(function (err) {
