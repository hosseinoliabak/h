...
description: "In the previous notes you have seen matrices as arrays of numbers that represent systems of linear equations. But there is another very powerful and..."
date: "2026-04-05"
difficulty: 3
order: 6
...
for i in range(-4, 5):
    for j in range(-4, 5):
        ...
        poly = Polygon(corners, closed=True, fc='#FFB347', ec='#CC7000', lw=0.5, alpha=0.35)
        ax.add_patch(poly)
...
viewof animBtn = {
  const btn = html`<button style="background:linear-gradient(135deg,var(--site-accent),var(--site-accent-hover));color:var(--site-accent-contrast);border:none;border-radius:6px;padding:6px 18px;font-size:13px;font-weight:600;cursor:pointer">▶ Animate</button>`;
  let count = 0;
  btn.value = count;
...
pts = np.array([[0, 0], [1, 0], [1, 1], [0, 1]])
pts1 = (A1 @ pts.T).T
pts2_only = (A2 @ pts.T).T
pts_combined = (A3 @ pts.T).T

configs = [
    (axes[0, 0], pts, '#87CEEB', '#4682B4', 'Unit square'),
    (axes[0, 1], pts1, '#FFB347', '#CC7000', r'$A_1$'),
    (axes[1, 0], pts2_only, '#B19CD9', '#6A0DAD', r'$A_2$'),
    (axes[1, 1], pts_combined, '#90EE90', '#2E8B57', r'$A_2 \cdot A_1$'),
]
...
configs = [
    (axes[0], pts, '#87CEEB', '#4682B4', 'Unit square'),
    (axes[1], pts_t, '#FFB347', '#CC7000', '$A$'),
    (axes[2], pts_back, '#87CEEB', '#4682B4', r'$A^{-1} \cdot A = I$'),
]
...
1. Assign a **score** to the word "lottery" and a score to the word "win"
2. Calculate the score of an email by adding the word scores with repetition
3. If the total score is bigger than some **threshold**, classify the email as spam
...
data = [(1,1,1), (2,1,1), (0,0,0), (0,2,1), (0,1,0), (1,0,0), (2,2,1), (2,0,1), (1,2,1)]
...
The dimension rule applies: the number of columns of the first matrix must match the number of rows of the second. Reversing the order here fails because $B$ is $3 \times 2$ and $A$ is $3 \times 3$:
...
This is convenient but can be surprising. If you explicitly reshape them as column vectors ($3 \times 1$), the multiplication fails because the dimensions no longer align:
...
`np.dot()` also works for matrix multiplication (it broadcasts the dot product across all rows and columns):

```{python}
print("np.dot(A, B):\n", np.dot(A, B))
```

NumPy also broadcasts scalars. Subtracting a scalar from a matrix is not defined mathematically, but NumPy expands the scalar to match the matrix shape and performs element-wise subtraction:
...
## Lab 2: Linear Transformations and Computer Graphics
...
Both properties hold, confirming $T$ is linear.
...
The columns of $A$ are the images of the standard basis vectors: $A = [L(\mathbf{e}_1) \quad L(\mathbf{e}_2)]$.
...
Let's apply transformations to a leaf shape stored as a set of 329,076 coordinate points, available on Google Drive as [image.txt](https://drive.google.com/file/d/1gGLMkRpjEE2NSv3a2uzym5fQqLXB46ga/view?usp=sharing) (16 MB). Each point is a vector $\begin{bmatrix}x\\y\end{bmatrix}$ in the plane, and transforming the entire image is just one matrix multiplication.
