...
**2. Which of the following vectors has the greatest norm?**
...
**3. Given the vectors $\vec{v} = (1, 0, 7)$ and $\vec{w} = (0, -1, 2)$, find the distance between them, $d(\vec{v}, \vec{w})$.**
...
**4. You are given the points $P = (1, 0, -3)$ and $Q = (-1, 0, -3)$. The magnitude of the vector from $P$ to $Q$ is:**
...
**5. Select the correct statements pertaining to the dot product.**
...
**6. Calculate the dot product $\vec{a} \cdot \vec{b}$ where $\vec{a} = \begin{bmatrix} 3 \\ 7 \\ 1 \end{bmatrix}$ and $\vec{b} = \begin{bmatrix} 4 \\ 0 \\ 3 \end{bmatrix}$.**
...
**7. Calculate the dot product $\vec{w} \cdot \vec{z}$ where $\vec{w} = \begin{bmatrix} -9 \\ -1 \end{bmatrix}$ and $\vec{z} = \begin{bmatrix} -3 \\ -5 \end{bmatrix}$.**
