...
Instead of writing one equation per row, we can bundle everything into matrices and vectors. The weight row vector $W$, input matrix $X$, and predicted output $\hat{y}$ are defined as:
...
Multiplying $W$ by each column of $X$ (one column = one training example) and adding the bias $b$ gives the entire system in one compact expression:
...
- $A$, $B$, $C$: capital letters represent matrices; $u$, $v$, $w$: lowercase represent vectors
- $X \in \mathbb{R}^{m \times n}$: $m$ rows (training examples), $n$ columns (features)
- $\mathbb{R}^{m \times n}$: a matrix of real numbers with $m$ rows and $n$ columns
