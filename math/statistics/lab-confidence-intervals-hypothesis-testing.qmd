...
axes[0].scatter(df['trip_seconds'], df['fare'], alpha=0.1, s=1, color='#4682B4')
axes[0].set_xlabel('Trip Duration (seconds)')
axes[0].set_ylabel('Fare ($)')
axes[0].set_title('Fare vs Trip Duration')

# Fare vs trip_miles
axes[1].scatter(df['trip_miles'], df['fare'], alpha=0.1, s=1, color='#4682B4')
...
Both plots show a positive relationship: longer trips and farther trips tend to cost more. There are some extreme outliers, but the general trend is clear. This suggests a linear model could work reasonably well.
...
ax.plot([0, df['fare'].max()], [0, df['fare'].max()], color='#CC0000', linestyle='--', linewidth=2, label='Perfect prediction')
...
The points cluster around the diagonal, which means the model is doing a reasonable job. However, you can see that the predictions are not perfect, especially for very expensive trips. This is normal: no model captures every real-world factor. Other variables like time of day, surge pricing, or route taken also affect the fare but are not included in our simple two-variable model.
