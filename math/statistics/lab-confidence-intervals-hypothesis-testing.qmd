...
Next we load the Chicago rideshare dataset, the same cleaned file used in the [second EDA lab](/math/probability/lab-rideshare-eda-w2.qmd). It is too large to host on this site, so download it from Google Drive: [rideshare_2022_cleaned.csv](https://drive.google.com/file/d/1CBJ3_65KzPuR5JWjeWe3cTuSlKyaAeFi/view?usp=sharing) (82 MB). The `parse_dates` argument tells pandas to read the `trip_start_timestamp` and `date` columns as proper date objects rather than plain text. This makes it easy to filter by date ranges and group by day later on.
...
Finally, let us visualize how well the model fits by plotting the original data and the model's predictions against trip duration. If the model captures the relationship well, the prediction cloud should follow the trend of the original data.
